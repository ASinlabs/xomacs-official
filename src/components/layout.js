...
const Layout = () => {
  ...
  const CaseStudiesCountStuffs = nodes.filter(
    nd => nd.title === "Case Studies Count"
  )

  const PostsArticlesStuffs = nodes.filter(
    nd => nd.title === "Posts & Articles"
  )
  ...
  const {
    pageBuilders: { services },
  } = ServicesStuffs[0]

  const {
    pageBuilders: { caseStudies },
  } = CaseStudiesStuffs[0]

  const {
    pageBuilders: { caseStudiesCount },
  } = CaseStudiesCountStuffs[0]

  const {
    pageBuilders: { postsArticles },
  } = PostsArticlesStuffs[0]

  const {
    pageBuilders: { technologies },
  } = TechnologiesStuffs[0]

  const {
    pageBuilders: { testimonials },
  } = CustomerFeedbackStuffs[0]
